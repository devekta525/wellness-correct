...
export default function ProductCard({ product, viewMode = 'grid' }) {
  ...
  const IMAGE_COUNT = Math.max(images.length, 1);
  ...
  return (
    <div className={`product-card group relative bg-white dark:bg-slate-800/90 rounded-3xl overflow-hidden shadow-sm border border-slate-100 dark:border-slate-700/50 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-100/60 dark:hover:shadow-blue-900/40 min-w-0 ${isList ? 'flex flex-col sm:flex-row' : 'flex flex-col'}`}>

      {/* ── Image Section ── */}
      <div className={`relative bg-slate-100 dark:bg-slate-800 ${isList ? 'w-full sm:w-48 h-48 sm:h-auto min-h-[200px] flex-shrink-0' : 'w-full aspect-[4/3]'} overflow-hidden`}>
        {IMAGE_COUNT > 0 ? (
            ...
            >
              {images[idx] ? (
                <Image
                  ...
                  sizes={isList ? '192px' : '(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw'}
                />
              ) : (
        ...
        >
          <Heart className={`w-4 h-4 transition-all ${isInWishlist(productId) ? 'fill-red-500 text-red-500' : ''}`} />
        </button>

        {/* Badge Overlay */}
        {product.badge && (
          <Badge className="absolute top-3 left-3 z-30 bg-gradient-to-r from-blue-600 to-indigo-600 text-white border-0 shadow-lg pointer-events-none">
            {product.badge}
          </Badge>
        )}
        ...
        {isOutOfStock && (
          <div className="absolute inset-0 z-20 bg-black/50 flex items-center justify-center backdrop-blur-[2px]">
            <span className="text-white font-bold tracking-wide px-4 py-2 border-2 border-white/50 rounded-xl backdrop-blur-md">OUT OF STOCK</span>
          </div>
        )}
      ...
      {/* ── Content ── */}
      <div className={`p-5 flex flex-col flex-grow ${isList ? 'justify-center' : ''}`}>
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-1">
            {[...Array(5)].map((_, i) => (
              <Star
                key={i}
                className={`w-3 h-3 ${i < (product.rating || 5) ? 'fill-amber-400 text-amber-400' : 'text-slate-200 fill-slate-100 dark:text-slate-700 dark:fill-slate-800'
                  }`}
              />
            ))}
            <span className="text-[11px] text-slate-400 dark:text-slate-500 ml-1 font-medium">({product.reviews || 0})</span>
          </div>
        </div>
        ...
        {/* Tagline / Description */}
        {(product.tagline || product.description || product.shortDescription) && (
          <p className="text-[12px] text-slate-500 dark:text-slate-400 mb-4 line-clamp-2">
            {product.tagline ? <span className="italic">{product.tagline}</span> : (product.shortDescription || product.description)}
          </p>
        )}

        {/* Price row */}
        <div className="flex items-center flex-wrap gap-2.5 mb-4">
          <span className="text-xl font-extrabold bg-gradient-to-r from-blue-600 to-cyan-600 dark:from-blue-400 dark:to-cyan-400 bg-clip-text text-transparent">
          ...
          <div className="space-y-2 mb-5">
            {product.for && (
              <div className="flex items-start gap-2">
                <span className="flex-shrink-0 mt-0.5 text-[9px] font-extrabold text-blue-500 bg-blue-50 dark:bg-blue-900/30 dark:border-blue-800 border border-blue-100 px-2 py-0.5 rounded-lg tracking-wide uppercase">
                  FOR
                </span>
                <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed font-medium">{product.for}</p>
              </div>
            )}
            {product.with && (
              <div className="flex items-start gap-2">
                <span className="flex-shrink-0 mt-0.5 text-[9px] font-extrabold text-cyan-600 bg-cyan-50 dark:bg-cyan-900/30 dark:border-cyan-800 border border-cyan-100 px-2 py-0.5 rounded-lg tracking-wide uppercase">
                  WITH
                </span>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed">{product.with}</p>
              </div>
            )}
          </div>
        )}
        ...
        <div className="flex gap-2.5">
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-[12px] font-bold transition-all duration-300 ${isOutOfStock
              ...
              : quantityInCart > 0
                ? 'bg-gradient-to-r from-emerald-500 to-teal-500 text-white shadow-lg shadow-emerald-200/50 hover:shadow-emerald-300/50 hover:scale-[1.02]'
                : 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:from-blue-700 hover:to-indigo-700 hover:shadow-lg hover:shadow-blue-200/50 dark:hover:shadow-blue-900/50 hover:scale-[1.02]'
              }`}
          >
            {quantityInCart > 0 ? (
              <span className="flex items-center gap-1.5">✓ Added ({quantityInCart})</span>
            ) : (
              <>
