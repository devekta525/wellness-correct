...
const ProductDetailPage = () => {
  ...
  useEffect(() => {
    ...
    fetch('http://127.0.0.1:7436/ingest/62e2a1c9-8294-48a2-981c-e3fb6efe754a', {
      ...
      body: JSON.stringify({
        sessionId: '10b514',
        runId: 'product-darkmode-pre-fix-1',
        hypothesisId: 'H1',
        location: 'ProductDetailPage.jsx:dark-mode-check',
        message: 'Product detail dark mode state',
        data: { isDark },
        timestamp: Date.now(),
      }),
    }).catch(() => {});
    ...
    if (descEl) {
      ...
      fetch('http://127.0.0.1:7436/ingest/62e2a1c9-8294-48a2-981c-e3fb6efe754a', {
        ...
        body: JSON.stringify({
          ...
          runId: 'product-darkmode-pre-fix-1',
          hypothesisId: 'H2',
          location: 'ProductDetailPage.jsx:description-styles',
          message: 'Description text and background colors',
          data: {
            color: style.color,
  ...
  if (!product) return <div className="page-container py-16 text-center"><h2 className="text-2xl font-bold text-gray-600 dark:text-gray-200">Product not found</h2></div>;
  ...
  return (
    <div className="page-container py-8 animate-fade-in">
      {/* Breadcrumb */}
      <nav className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-4 sm:mb-6 overflow-hidden">
        <Link to="/" className="hover:text-primary-600 dark:hover:text-primary-400 truncate shrink-0">Home</Link>
        <ChevronRight size={14} className="flex-shrink-0" />
        <Link to={`/category/${product.category?.slug}`} className="hover:text-primary-600 dark:hover:text-primary-400 truncate">{product.category?.name}</Link>
        <ChevronRight size={14} className="flex-shrink-0" />
        <span className="text-gray-700 dark:text-gray-200 truncate min-w-0">{product.title}</span>
      </nav>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-8 md:mb-12">
        ...
        <div className="space-y-5">
          {product.brand && <p className="text-sm font-medium text-primary-600 dark:text-primary-400 uppercase tracking-wider">{product.brand}</p>}
          ...
          {product.ratings?.count > 0 && (
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">{product.ratings.average.toFixed(1)}</span>
              <span className="text-sm text-gray-500 dark:text-gray-400">({product.ratings.count} reviews)</span>
            </div>
          )}

          {/* Price */}
          <div className="flex items-end gap-3">
            <span className="text-3xl font-black text-gray-900 dark:text-gray-100">₹{discountedPrice.toFixed(2)}</span>
            {product.discount > 0 && (
              <>
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3">
            ...
            <button onClick={handleShare}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-all text-sm font-medium">
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3 pt-2">
            {[
              ...
              <div key={text} className="flex flex-col items-center text-center p-3 bg-gray-50 dark:bg-gray-900 rounded-xl">
                <Icon size={20} className="text-primary-600 mb-1" />
                <span className="text-xs font-semibold text-gray-800 dark:text-gray-100">{text}</span>
      ...
      {/* Tabs */}
      <div className="mb-8">
        <div className="flex gap-0 overflow-x-auto scrollbar-hide border-b border-gray-200 dark:border-gray-700 mb-6 -mx-1 px-1">
          {['description', 'features', 'reviews'].map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-3 sm:px-5 py-3 text-xs sm:text-sm font-semibold capitalize transition-all border-b-2 -mb-px whitespace-nowrap flex-shrink-0 ${
                activeTab === tab
        ...
        {activeTab === 'description' && (
          <div
            ref={descriptionRef}
            className="prose max-w-none text-gray-600 dark:text-gray-100 dark:prose-invert leading-relaxed"
