...
const BrandPage = () => {
  ...
  useEffect(() => {
    brandAPI.getBySlug(slug).then((res) => setBrand(res.data.brand)).catch(() => {});
  }, [slug]);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      setLoading(true);
      try {
        const res = await productAPI.getAll({ ...filters, brand: slug, limit: 20 });
  ...
  return (
    <div className="page-container py-8 animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{brand?.name || 'Brand'}</h1>
          {brand?.description && <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{brand.description}</p>}
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            className="input py-2 pr-8 text-sm w-40 bg-white dark:bg-gray-800"
          >
            {SORT_OPTIONS.map((o) => (
      ...
      {filterOpen && (
        <div className="card p-4 mb-6 animate-slide-up">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <label className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-1.5 block">Min Price (₹)</label>
            ...
            <div>
              <label className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-1.5 block">Max Price (₹)</label>
      ...
      {loading ? (
        ...
        <>
          ...
          <Pagination currentPage={filters.page} totalPages={pagination?.pages || 1} onPageChange={(page) => updateFilter('page', page)} />
        </>
      )}
    </div>
  );
